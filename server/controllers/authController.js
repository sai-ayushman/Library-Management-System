...
export const register = catchAsyncErrors(async (req, res, next ) => {
    try{
        ...
        const isRegistered = await User.findOne({email, acoountVerified: true});
        if(isRegistered){
            return next(new ErrorHandler("User Already exists", 400));
        }
        const registrationAttemptByUser = await User.find({
            email,
            acoountVerified: false,
        });
...
export const login = catchAsyncErrors(async (req, res, next )=>{
    ...
    sendToken(user, 200, "User login sucessefully.", res);
})
...
export const forgotPassword = catchAsyncErrors(async ( req, res, next) => {
    ...
    try {
        await sendEmail({
            email: user.email,
            subject: "BookNerd Library Management System Password Recovery",
            message,
        });
        res.status(200).json({
            success: true,
